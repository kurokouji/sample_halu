...
class AppCache():
    ...
    def getPrintServerControllerObject(self, requestdict):
        """
        プリントコントローラクラスを動的にインスタンス。
        サーバプログラムとサーバプログラム用モジュール(Sever/Commons)のパス追加も行う。
        ...
        exec(w_import)
        # クラスをインスタンス
        w_class = class_name + '(self.mvclog, self.mvclogname, requestdict)'
        app_object = eval(w_class)

        return app_object


    def getFormObject(self, pdfcreate, responsedict, sqldict, requestdict):
        """
        フォームクラスを動的にインスタンス。
        サーバプログラムとサーバプログラム用モジュール(Sever/Commons)のパス追加も行う。
        ...
        exec(w_import)
        # クラスをインスタンス
        w_class = class_name + '(self.mvclog, self.mvclogname, pdfcreate, responsedict, sqldict, requestdict)'
        app_object = eval(w_class)
