...
class Database():
    ...
    def commit(self):
        ...
        for key in self.trans:
            self.dlog.debug(self.dlogname, 'Database コミット処理 start')
    ...
    def rollback(self):
        ...
        """
        for key in self.trans:
            self.dlog.debug(self.dlogname, 'Database ロールバック処理 start')
    ...
    def close(self):
        ...
        for key in self.connect:
            self.dlog.debug(self.dlogname, 'Database クローズ処理 start')
